...
{
  // deal mmap lazy alloction
  struct proc *p = myproc();
  int i;
  for (i = 0; i < VMA_TOTAL; i++)
  {
  ...
  // 读取文件内容
  ilock(vf->ip);
  // 计算当前页面读取文件的偏移量，实验中p->vma[i].offset总是0，offset是文件的读写起点
...
void usertrap(void)
{
  ...
  {
    #ifdef LAB_MMAP
    uint64 fault_va = r_stval();// fault_va是页面错误的虚拟地址
